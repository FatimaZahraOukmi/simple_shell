...
/**
 * builtin_exit - exit of the prog with the status
...
/**
 * builtin_cd - change the current directory to another
 * @data: struct for the prog's data
 * Return: zero if sucess, or other number if its declared in the arguments
 */
...
/**
 * builtin_help - shows the environ where the shell runs
 * @data: struct for the prog's data
 * Return: zero if sucess, or other number if its declared in the arguments
 */
...
int builtin_alias(data_of_program *data)
{
	...
	/* if there are no args, print all environment */
	if (data->tokens[1] == NULL)
		return (print_alias(data, NULL));

	while (data->tokens[++i])
	{/* if there are args, set or print each env var*/
